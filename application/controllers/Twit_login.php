...
class Twit_login extends CI_Controller{
	...
	function close(){
		/*if(isset($_SESSION['twitter_login']) and $_SESSION['twitter_login']==1)
			...
			echo "<script>window.opener.success_twitter();</script>";
	}
...
//		session_start();
		...
		if (!empty($_GET['oauth_verifier']) && !empty($_SESSION['oauth_token']) && !empty($_SESSION['oauth_token_secret'])) {
			...
			if (isset($user_info->error)) {
			   ...
			   //$twitter_otoken_secret=$_SESSION['oauth_token_secret'];
				...
				$profile_picture = $user_info->profile_image_url;
				...
				if($check){
					...
					$table='user_tb';
					$data=array('tw_data'=>$social_data
					,'name'=>$name,'created_date'=>$newdate,'tw_id'=>$uid,'profile_picture'=>$profile_picture,'vserv' => $vserv);
	...
	function get_data(){
		$check=$this->user_model->get_user_by_login($this->session->userdata('user_id'));
		if($check){
			...
			echo json_encode(array('status'=>1,'nama'=>$check['name'],'email'=>$check['email'],
				'telp'=>$check['telp'],'tanggal'=>$tanggal,'produk'=>$check['product_susu_yang_digunakan']));exit();
		}
		else{
			echo json_encode(array('status'=>0,'nama'=>$this->session->userdata('name')));exit();
		}
