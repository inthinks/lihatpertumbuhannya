...
class Weight_calculation extends CI_Controller{
	...
	function index(){
		$this->data['weight'] = $this->calculation_model->get_weight();
		$this->data['content'] = 'pediasurePanel/weight_calculation/list';
		$this->data['gender']=1;
		$this->load->view('common/pediasurePanel/body', $this->data);
	}

	function female(){
		$this->data['weight'] = $this->calculation_model->get_weight_female();
		$this->data['content'] = 'pediasurePanel/weight_calculation/list';
		$this->data['gender']=0;
		$this->load->view('common/pediasurePanel/body', $this->data);
	}
	...
	function add($gender){
		...
		$this->data['content'] = 'pediasurePanel/weight_calculation/add';
		$this->load->view('common/pediasurePanel/body', $this->data);
	}
	...
	function do_add(){
		...
		$check_month = $this->calculation_model->check_month($month);
		...
		if($check_month){
			...
			redirect('pediasurePanel/weight_calculation');
		}
	}

	function edit($id){
		$this->data['id']=$id;
		$this->data['weight']=$this->calculation_model->get_weight_detail($id);
		$this->data['content'] = 'pediasurePanel/weight_calculation/edit';
		$this->load->view('common/pediasurePanel/body', $this->data);
	}
	...
	function delete($id){
		$this->general_model->delete_data('weight_calculation_tb',array('id'=>$id));
		redirect($_SERVER['HTTP_REFERER']);
	}
